...
let defaultLabelsWorkers = {}; //default labels you want to add to cluster worker metrics but not cluster aggregate metrics
...
class AggregatorRegistry extends Registry {
	...
	static setDefaultLabelsWorkers(labels) {
		defaultLabelsWorkers = labels;
	}
}
...
process.on('message', message => {
	if (cluster().isWorker && message.type === GET_METRICS_REQ) {
		Promise.all(registries.map(r => r.getMetricsAsJSON(defaultLabelsWorkers)))
			.then(metrics => {
				process.send({
