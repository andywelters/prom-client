...
class Registry {
	...
	async getMetricsAsJSON(labelsForCopy = {}) {
		const metrics = [];
		const defaultLabelNames = Object.keys(this._defaultLabels);
		const labelsForCopyNames = Object.keys(labelsForCopy);
		const promises = [];
		let newValue;
		let newValues;
		...
		for (const item of resolves) {
			if (item.values) {
				...
				//make a copy of each metric variant with the additional labels added
				if(labelsForCopyNames.length > 0) {
					newValues = [];
					for (const val of item.values) {
						// Make a copy before mutating
						newValue = Object.assign({}, val);
						newValue.labels = Object.assign({}, newValue.labels);
						for (const labelName of labelsForCopyNames) {
							val.labels[labelName] =
								val.labels[labelName] || labelsForCopy[labelName];
						}
						newValues.push(newValue);
					}
					item.values = item.values.concat(newValues);
				}
			}
